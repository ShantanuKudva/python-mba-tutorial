...
Lesson: 03-mba-analytics/marketing/lessons/02-cohorts.md
Difficulty: Hard
...
# 🛠️ Step 1: find each customer's acquisition cohort (their FIRST order period).
#    cohort_map = orders.groupby("customer_id")["order_period"].min()
#    cohort_map.name = "cohort"
#    orders = orders.join(cohort_map, on="customer_id")

# 🛠️ Step 2: compute period_index = order_period - cohort (in months).
#    orders["period_index"] = (
#        orders["order_period"].dt.start_time.dt.to_period("M").apply(int)
#        - orders["cohort"].dt.start_time.dt.to_period("M").apply(int)
#    )
#    Simpler alternative using ordinal:
#    orders["period_index"] = orders["order_period"].astype(int) - orders["cohort"].astype(int)

# 🛠️ Step 3: count distinct customers per (cohort, period_index).
#    retention = (
...
# 🛠️ Step 4: divide by the period_index=0 column to get retention rates.
