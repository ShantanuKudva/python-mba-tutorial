...
# 🛠️ Step 1: compute the present value of each FCF.
#    pv_fcfs = [fcf / (1 + wacc) ** (t + 1) for t, fcf in enumerate(fcfs)]

# 🛠️ Step 2: compute terminal value at end of year 5.
#    tv = fcfs[-1] * (1 + g) / (wacc - g)

# 🛠️ Step 3: discount TV back to today (5 years).
#    pv_tv = tv / (1 + wacc) ** len(fcfs)

# 🛠️ Step 4: enterprise value = sum of PV(FCFs) + PV(TV).
