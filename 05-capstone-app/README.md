...
End to end. Live URL at the end of week 12.
...
You **do** build the backend. Every endpoint starts as a stub returning mock data so the frontend works on day 1. Your job is to progressively replace each stub with the real pandas/Groq logic you wrote in weeks 6–10.
...
| Week | Focus |
|---|---|
| 11 | Wire `/upload` and `/analyze` — get real metrics flowing. |
| 12 | Wire `/summarize` (Groq) and `/report` (xlsx generation). Deploy. |
